...
//ставим прослушку событий на переключение языков
...
search.addEventListener("input", function () {
  ...
  const itemCard = document.querySelectorAll(".info-valute_item");

  if (val !== "") {
    for (let i of itemCard) {
      console.log(i.childNodes[1].childNodes[3].childNodes[3].textContent);
      if (
        i.childNodes[1].childNodes[1].childNodes[1].innerText
          .toUpperCase()
          .search(val) == -1 &&
        i.childNodes[1].childNodes[3].childNodes[3].innerText
          .toUpperCase()
          .search(val) == -1
      ) {
        i.classList.add("none");
